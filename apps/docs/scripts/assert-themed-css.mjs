 ...
 * only the Satoshi @font-face rules — because `nextra-theme-docs/style.css`
 ...
 *
 * Markers are stable strings from nextra-theme-docs@4.6.1's compiled
 * `dist/style.css` (pinned dependency, bundled wholesale by the import): the
 * `@theme` block's primary scale and the `.nextra-hamburger` structural class.
...
const REQUIRED_MARKERS = ['--x-color-primary-', '.nextra-hamburger']
...
if (total < MIN_TOTAL_BYTES || missing.length > 0) {
  console.error(
    `[assert-themed-css] FAIL — exported CSS does not contain the nextra-theme-docs stylesheet.\n` +
      `  total bytes: ${total} (floor ${MIN_TOTAL_BYTES})\n` +
      `  missing markers: ${missing.join(', ') || 'none'}\n` +
      `  Likely cause: \`import 'nextra-theme-docs/style.css'\` removed from app/layout.tsx.`,
  )
  process.exit(1)
}
