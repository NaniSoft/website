...
- "Built in-house" strip (4): **Atlas** — the core engine: traversal API, policy enforcement, audit log. **Compass** — the traversal UI: explore the twin as a graph. **DataGerry Bridge** — glue that syncs authored schema into the lakehouse and the engine. **Scout** — connectors for internal systems no catalog covers.
