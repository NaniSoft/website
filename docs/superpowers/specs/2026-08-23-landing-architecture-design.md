...
- **Layout** (derived, not hardcoded — same derivation logic as the playground module, smaller scale): 8 stage columns from `PIPELINE_SPINE` × `STAGE_COMPONENTS` (columns vertically centered; two presentation-only row-order overrides align flows — Trailhead centered between its trigger targets, Overlook on the lower serving row); Watchtower observer above at the mean x of its observe targets, nudged half a column pitch right so its drop-legs fall in a column gap rather than on a chip; Anchor/Conveyor/OpenBao platform band above the spine (platform-kind minus observer), spread evenly in an explicit slot order — OpenBao left, Anchor central, Conveyor outermost-right — because a mean-target sort would strand Conveyor left of Anchor and force its feeders through Anchor's chip (as-built refinement of the playground rule, recorded 2026-08-23); persona edges dropped (endpoints not rendered — same as playground). Phase bands below, grouped from consecutive columns sharing a phase: Sources (subtle) | Schema | Ingestion | Transform | Investigation.
...
- **Status reducer** `deriveSectionState(activeIdx)` — pure: given active phase index 0..3 returns, for every band/node/edge, `idle | active | done` using playground semantics (jade = active, teal = done). Node/edge membership maps to phases via the same column grouping as the bands. Edge semantics (as-built refinement of the "either endpoint active" draft): each edge's position is the **later of its endpoints'** pipeline positions — the edge lights jade in the phase where its data lands, then settles teal — which keeps every edge's lifecycle monotone (idle → active for exactly one phase → done). Source systems read as position −1: never active, teal-done once ingestion has run; cross-cutting platform/observer nodes and their dotted edges stay neutral at every phase.
...
- **Visual language (SPEC §2 hero graph language, token discipline):**
  - SVG scaled responsively (`width: 100%`, fixed viewBox). Chips: rect rx 12 (inner radius), codename in Satoshi 600 + realName in JetBrains Mono muted (the data face for data-shaped labels); over-wide realNames glyph-squeeze to the chip width via `textLength`.
