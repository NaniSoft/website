...
function getDemoScenarios(): Scenario[] {
  return [
    {
      key: 'support',
...
function formatPlanPrice(price: string, cycle: BillingCycle): string {
  if (price === 'Custom') {
    return price
  }
...
export function InteractiveDemoPanel({ ctaText }: InteractiveDemoProps) {
...
export function InteractivePricingCards({ plans }: InteractivePricingProps) {
      ...
      <ul className="grid gap-4 lg:grid-cols-3">
        {plans.map(plan => (
          ...
          >
            ...
            <p className="mt-1 text-2xl font-semibold text-slate-900 dark:text-zinc-100">{formatPlanPrice(plan.price, cycle)}</p>
